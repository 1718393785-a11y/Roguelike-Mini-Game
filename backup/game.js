...
class FireTornado {
    ...
    render(ctx) {
        ...
        if (this.mode === 'massive_storm') {
            // 巨型聚变风暴：红橙色烈焰
            strokeColor = '#ff2200';
            fillColor = `rgba(255, 60, 0, ${0.3 + 0.2 * Math.random()})`;
            shadowColor = '#ff3300';
            lineWidth = 8;
        } else if (this.mode === 'seek_player') {
            // 向玩家移动的聚变龙卷风：偏亮橙色
            strokeColor = '#ff6600';
            fillColor = `rgba(255, 100, 0, ${0.25 + 0.15 * Math.random()})`;
            shadowColor = '#ff6600';
            lineWidth = 4;
        } else {
            // 普通索敌：原有配色 暗金+紫红
            strokeColor = '#b8860b';
            fillColor = `rgba(255, 0, 255, ${0.3 + 0.2 * Math.random()})`;
            shadowColor = '#b8860b';
        }
        ...
        if (needProximity) {
            this.spawnQueue.push({
                ...
                action: () => {
                    ...
                    const gm = window.gameManager;
                    const margin = 150;
                    const spawnX = margin + Math.random() * (gm.canvas.width - margin * 2);
                    const spawnY = margin + Math.random() * (gm.canvas.height - margin * 2);
                    const tornado = new FireTornado(
                        spawnX, spawnY,
                        this.baseRadius,
                        this.baseLifetime,
...
class Player {
    constructor(canvasWidth, canvasHeight, perks, perkLevels) {
        ...
        const picked = weaponChoices[Math.floor(Math.random() * weaponChoices.length)];
...
class WoodenOxEnemy extends Enemy {
    constructor(x, y, baseHealth) {
        ...
        this.directionChangeTimer = 1.5 + Math.random() * 1;
    }
    ...
    update(deltaTime, canvasWidth, canvasHeight) {
        ...
        if (this.directionChangeTimer <= 0) {
            // 随机抖动方向 ±30度
            this.dirAngle += (Math.random() - 0.5) * Math.PI / 3;
            this.directionChangeTimer = 1.5 + Math.random() * 1;
        }
...
class Pickup {
    ...
    onPickup(gameManager) {
        if (this.type === PICKUP_TYPES.EXP) {
            ...
            const finalCurrency = baseGained + (Math.random() < fraction ? 1 : 0);
...
class GameManager {
    ...
    spawnEnemy() {
        ...
        const isArcher = Math.random() < 0.10; // 10% 概率刷出远程弓手
        const isElite = !isArcher && Math.random() < 0.05;
        const isOx = !isArcher && Math.random() < 0.02; // 2% 概率刷出盗宝贼
        ...
        if (isArcher) {
            ...
            if (hasExistingOx) {
                ...
                if (Math.random() < 0.5) {
                    x = Math.random() < 0.5 ? innerMargin : this.canvas.width - innerMargin;
                    y = Math.random() * (this.canvas.height - innerMargin * 2) + innerMargin;
                } else {
                    y = Math.random() < 0.5 ? innerMargin : this.canvas.height - innerMargin;
                    x = Math.random() * (this.canvas.width - innerMargin * 2) + innerMargin;
                }
    ...
    spawnMiniBoss(minute) {
        ...
        const miniBoss = new EliteEnemy(
            this.canvas.width / 2 + (Math.random() > 0.5 ? 200 : -200),
            this.canvas.height / 2 + (Math.random() > 0.5 ? 200 : -200),
            hp,
            1.2
        );
    ...
    spawnSpearmanSwarm(wave) {
        ...
        const countPerLayer = 20 + Math.floor(Math.random() * 10);
    ...
    spawnNextSwarmLayer() {
        ...
        for (let i = 0; i < ps.countPerLayer; i++) {
            ...
            const isCavalry = Math.random() < (ps.isCavalryMain ? 0.8 : 0.2);

            let baseHp, speedMod, type;
            if (isCavalry) {
                baseHp = (3 + ps.wave * 1);
                speedMod = 2.5 + Math.random() * 0.5;
                type = 'cavalry';
                ps.cavalryCount++;
            } else {
                baseHp = (5 + ps.wave * 2);
                speedMod = 2.0 + Math.random() * 0.2;
                type = 'spearman';
                ps.spearmanCount++;
            }
    ...
    generateLevelUpOptions() {
        ...
        if (hasAnyWeaponInPool && !hasAnyWeaponInSelected) {
            ...
            if (weaponOptions.length > 0) {
                // 随机选一个武器选项，替换掉最后一个选项
                const randomIdx = Math.floor(Math.random() * weaponOptions.length);
                selected.pop(); // 移除最后一个
    ...
    update(deltaTime) {
        ...
        if (!hasFinalBoss && propCount < maxProps && Math.random() < 0.001) {
                            ...
                            if (enemy.isBoss && originalIdx >= 0) {
                                if (this.currentStage >= STAGES.length - 1) {
                                    ...
                                    this.targetEnemyCount = Math.floor(
                                        ...
                                        Math.random() * (STAGES[this.currentStage].maxSpawnCount - STAGES[this.currentStage].minSpawnCount)
                                    );
    ...
    renderVictory() {
        ...
        for (let i = 0; i < 50; i++) {
            ...
            if (isDigital) {
                ctx.fillStyle = `rgba(0, 255, 204, ${0.1 + Math.random() * 0.3})`;
                ctx.font = '12px "Courier New"';
                ctx.fillText(Math.random() > 0.5 ? '1' : '0', Math.abs(px), py);
            } else {
